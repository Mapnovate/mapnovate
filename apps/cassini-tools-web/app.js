...
(function () {
  ...
  function cassiniUnitLabel() {
    return $('cunit').value === 'm' ? 'X  Y in metres' : 'X  Y in feet';
  }

  function updateLabels() {
    if (dir === 'c2u') {
      $('lblIn').children[0].textContent = 'Input — Cassini';
      $('unitIn').textContent = cassiniUnitLabel();
      ...
      $('unitOut').textContent = cassiniUnitLabel();
  ...
  // refresh unit labels without clearing when only the unit changes
  function refreshUnitLabels() {
    if (dir === 'c2u') $('unitIn').textContent = cassiniUnitLabel();
    else $('unitOut').textContent = cassiniUnitLabel();
  }
  $('cunit').addEventListener('change', refreshUnitLabels);

  // ---- sample loader ----
  $('loadSample').addEventListener('click', () => {
    const key = dir === 'c2u' ? '39S' : '37S';
    const pts = net.groups[key].slice(0, 4);
    const fromFt = $('cunit').value === 'm' ? CE.FT : 1;   // control data is in feet
    if (dir === 'c2u')
      $('input').value = pts.map(p => `${(p[0]*fromFt).toFixed(3)}  ${(p[1]*fromFt).toFixed(3)}`).join('\n');
    else
  ...
  function run() {
    ...
    // Cassini units: engine works in feet internally, so convert at the boundary.
    const cUnit = $('cunit').value;                 // 'm' | 'ft'
    const toFt = cUnit === 'm' ? (1 / CE.FT) : 1;   // input Cassini -> feet
    const fromFt = cUnit === 'm' ? CE.FT : 1;       // engine feet -> output Cassini
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      try {
        if (dir === 'c2u') {
          // Cassini input -> feet for the engine
          const aFt = a * toFt, bFt = b * toFt;
          let cm = cmFixed, south = southFixed;
          if (cm == null || south == null) {
            const [dcm, dsouth] = CE.detectBeltFromCassini(aFt, bFt, net, southFixed);
            cm = cm == null ? dcm : cm;
            south = south == null ? dsouth : south;
          }
          const [e, n, near] = CE.convert(aFt, bFt, cm, south, 'c2u', useCtrl ? net : null, useCtrl);
          ...
          // engine returns Cassini feet -> selected output unit
          out.push(`${(x*fromFt).toFixed(3)}  ${(y*fromFt).toFixed(3)}   [${cm}°${south?'S':'N'}]`);
